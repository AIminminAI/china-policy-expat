// 政策匹配计算逻辑 - 从 api/calculate.ts 提取
// 供前端直接调用，不依赖 Vercel 运行时

import { policies, type Policy } from "../data/policies-full";
...
// 根据用户信息匹配所有政策并生成总结
export function calculateMatchedPolicies(
  request: CalculateRequest
): { matchedPolicies: MatchedPolicy[]; summary: string } {
  // 匹配所有政策（全部免费）
  ...
  return { matchedPolicies, summary };
}
